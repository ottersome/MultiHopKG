...
### BERT-conditioned traversal (optional)
...
- `question_texts_path` optionally points to a TSV/JSONL mapping of `(e_s, r)` pairs to question text. If omitted, a zero vector is used as a placeholder.

Notes:
- If HuggingFace Transformers are available, BERT encodings are used; otherwise the code falls back to deterministic pseudo/zero embeddings to keep the pipeline runnable.
