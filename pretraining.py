...
class TrainLive:
    def __init__(self, num_epochs, num_batches):
        ...
        self.epoch_task = self.progress.add_task("[red]Training", total=num_epochs)
        self.batch_task = self.progress.add_task("[green]Batches", total=num_batches)

        self.table = Table(title="Training")
        self.table.add_column("Train CELoss", style="magenta")
        self.table.add_column("Val CELoss", style="magenta")
        self.table.add_row("0", "0")

        self.layout.split(
            Layout(self.progress, name="progress", size=3),
            Layout(self.table, name="table")
        )
    ...
    #                 time.sleep(0.1)
    #             
    #     self.thread = threading.Thread(target=run_live, daemon=True)
    #     self.thread.start()

    def update_batch(self, table_stats: List, epoch_no:int, batch_no: int):

        # if len(table_stats) != 0:
        #     self.table.rows.clear()
        table_stats_str = [str(stat) if not isinstance(stat, str) else stat for stat in table_stats]
        self.table.add_row("bah","lah")
        #
        # self.progress.update(self.epoch_task, completed=epoch_no)
        self.progress.update(self.batch_task, completed=batch_no)

        # Notify layout that children changed
        # self.layout["progress"].update(self.progress.get_renderable())
        # self.layout["table"].update(self.table)
